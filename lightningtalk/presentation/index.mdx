...
import cloudRunLog from '../assets/cloud-run-log.png';
...
# For example, here is a <Link href="https://dash-gallery.plotly.host/dash-daq-satellite-dashboard/">dashboard of satellite telemetry</Link>.
## たとえば、これは衛星遠隔測定法のダッシュボードです。
...
# The preceeding, as well as subsequent Dash tutorials... 
...
<Layout>
  <Fill style={{ margin: "auto" }}>
    <Heading size={3}>In asia-northeast-1 region, average response time seems to about 10ms</Heading>
    <Heading size={4}>asia-northeast-1地域では、平均応答時間は約10ミリ秒ぐらいです。</Heading>
  </Fill>
  <Fill>
    <Image style={{ borderRadius: "5px", marginLeft: "30vmin", height: "85vmin" }} src={cloudRunLog} />
  </Fill>
</Layout>
